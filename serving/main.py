...
from serving.models import Base, Prediction, Feedback, Team
...
class PredictRequest(BaseModel):
    description: str = Field(
        ...,
        json_schema_extra={
            "example": "OAuth token validation failure on user login endpoint"
        },
    )
...
async def predict_single(req: PredictRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        predictions_counter.labels(
            team=result["prediction"], confidence_level=conf_level
        ).inc()
...
async def get_history(
    ...
    total = query.count()
    results = (
        query.order_by(Prediction.timestamp.desc()).offset(offset).limit(limit).all()
    )
...
async def list_teams(db: Session = Depends(get_db)):
    ...
    if not teams:
        ...
        return {
            "success": True,
            "count": len(default_teams),
            "teams": [{"id": code, "name": name} for code, name in default_teams],
        }
...
async def get_metrics(db: Session = Depends(get_db)):
    ...
    return {
        ...
        "high_confidence_pct": (high_confidence / max(total_predictions, 1) * 100)
