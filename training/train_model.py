...
from tensorflow.keras.layers import Dense, Dropout, Input, Activation, PReLU, LeakyReLU
...
from MLFunctions import (
    plot_history,
    test_with_uncertainty,
    get_monte_carlo_accuracy,
    graph_confidence,
    acc_to_uncertainty,
)
...
def process_data(csv: str, data_dir: str) -> pd.DataFrame:
    ...
    ## DATA CLEANING ##
    # 1. Eliminate:
    assigned_bugs = email_bugs[
        email_bugs["backlog_id"].notnull()
    ]  # bugs that were not assigned to a backlog
    assigned_bugs = assigned_bugs[
        assigned_bugs["team_id"].notnull()
    ]  # bugs that lack a team
    assigned_bugs = assigned_bugs[
        assigned_bugs["description"].notnull()
    ]  # bugs that lack a description
    assigned_bugs = assigned_bugs[assigned_bugs["label"].notnull()]  # or label

    # 2. Drop Duplicates:
    assigned_bugs.drop_duplicates(
        subset="work_id", keep=False, inplace=True
    )  # duplicate bugs
...
def fit_tfidf(assigned_bugs: pd.DataFrame, model_dir: str):
    ...
    train_features, test_features, train_labels, test_labels = train_test_split(
        assigned_bugs[category], backlog_labels, test_size=0.2
    )
    ...
    train_features = pd.DataFrame(
        tfidf_vectorizer.fit_transform(train_features).toarray()
    )  # Fit the Vectorizer to the train data
    test_features = pd.DataFrame(
        tfidf_vectorizer.transform(test_features).toarray()
    )  # Only transform (don't fit) the test data to emulate real-world predictions
    ...
    logging.warning(
        "TFIDF Vectors may grow to an astronomical size over time "
    ...
    logging.info(
        "Current number of backlogs this classifier will classify into :"
        + str(train_labels.shape[1])
    )
...
def build_tfidf_model(
    ...
    model.compile(
        loss="kullback_leibler_divergence", optimizer=optimizer, metrics=["accuracy"]
    )
...
def train_model(
    ...
    for i in range(num_trials):
        ...
        assignment_model = build_tfidf_model(
        ...
        early_stop = keras.callbacks.EarlyStopping(
            monitor="val_accuracy", restore_best_weights=True, patience=30
        )
        ...
        preds_df = test_with_uncertainty(
            assignment_model, test_features, test_labels, label_to_id, n_iter=100
        )
        _, test_accuracy = get_monte_carlo_accuracy(
            preds_df=preds_df, threshold=None
        )  # get overall acc
        ...
        if desired_acc is not None:
            threshold = acc_to_uncertainty(
                preds_df, desired_acc
            )  # get the uncertainty threshold that produces
            # the desired acc
            data_retained, _ = get_monte_carlo_accuracy(
                preds_df=preds_df, threshold=threshold
            )
        ...
        # If no desired acc is found, select most accurate model overall:
            ...
            if best_model:
                max_test_acc = test_accuracy
                max_val_acc = (
                    validation_acc  # record val acc also for record keeping purposes
                )
        ...
        if best_model:
            ...
            print(
                "- Test accuracy (no confidence bounds): %", test_accuracy, file=results
            )

            print("\n## Example Predictions:", file=results)
            print(
                tabulate(preds_df[:10], tablefmt="pipe", headers="keys"), file=results
            )
            ...
            print(
                "\n## Efficiency/ Accuracy Trade-Off (Confidence Bounding):",
                file=results,
            )
    ...
    logging.info(
        "Threshold uncertainty value to be used in production: " + str(final_threshold)
    )

    if desired_acc is not None:
        logging.info(
            "% of emails the model could confidently route using the desired accuracy: "
...
def training_process(training_csv):
    ...
    train_features, test_features, train_labels, test_labels = fit_tfidf(
        assigned_bugs, model_dir
    )
    ...
    num_trials = (
        1  # train the model only once (for speed) in the production environment
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="Automated Training File for TFIDF Email Classifier",
        description=description,
    ...
    parser.add_argument(
        ...
        "--file",
        type=str,
        help="The name of csv file containing bug descriptions and assignments",
        dest="csv",
    )
    parser.add_argument(
        "-p",
        "--pickle",
        type=str,
        help="The name of pickle file containing preprocessed bug descriptions and their assignments",
        dest="pickle",
    )
    parser.add_argument(
        "-md",
        "--model-dir",
        type=str,
        help="Use this option if you'd like to specify a model directory other than the default",
        dest="model_dir",
    )
    parser.add_argument(
        "-dd",
        "--data-dir",
        type=str,
        help="Use this option if you'd like to specify a data directory other than the default ",
        dest="data_dir",
    )
    ...
    if not args.csv and not args.pickle:
        parser.print_help()
        print(
            "No CSV or Pickle File Found. Please pass a csv or pickle file to train the model."
        )
        sys.exit(1)
        ...
        parser.print_help()
        print(
            "Passed Both CSV and Pickle. Please pass only one file to this training script"
        )
        sys.exit(1)
    ...
    num_trials = (
        1  # train the model only once (for speed) in the development environment
    )
    if config["APP"]["ENVIRONMENT"] == "production":
        num_trials = 10
