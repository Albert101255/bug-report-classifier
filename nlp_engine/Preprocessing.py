...
def preprocess_training_text(
    ...
    for token in doc:
        ...
        if (
            remove_num is True
            and (token.pos_ == "NUM" or token.text.isnumeric())
            and flag is True
        ):
            flag = False
...
def preprocess_training_text_with_stops(text, convert=False):
    ...
    return preprocess_training_text(
        text,
        accented_chars=True,
        convert_num=False,
        extra_whitespace=True,
...
def text_to_corpus(
    ...
    for token in doc:
        ...
        # remove punctuations
        if (
            punctuations is True
            and (token.pos_ == "PUNCT" and not token.tag_ == ".")
            and flag is True
        ):
            flag = False
        ...
        if token.tag_ == "." and flag is True:
            clean_text.append(".")

        # add text lemmas to the clean text:
        elif edit != "" and flag is True:
            clean_text.append(edit)
