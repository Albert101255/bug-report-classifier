...
def test_with_uncertainty(
    ...
    for i in range(len(predictions)):
        one_hot_label = test_labels.iloc[
            i
        ]  # Get the ground truth one-hot-encoded label for this prediction
        predicted_index = np.argmax(
            predictions[i]
        )  # Predicted index is maximum softmax ouput probability
        predicted_value = label_to_id[
            predicted_index
...
def predict_with_uncertainty(
    model, test_features: pd.DataFrame, label_to_id: dict, n_iter=10
) -> tuple:
    """
    USE FOR DEPLOYMENT
    ------------------
    ...
    test_features = test_features.values.reshape(
        1, -1
    )  # reshape single input sample to matrix so model can parse it
...
def graph_confidence(preds_df, save=False, fname=None):
    ...
    uncertanties = np.logspace(
        0, 10, 10000, base=0.5
    ).tolist()  # Logspace generates a better distribution of values since
