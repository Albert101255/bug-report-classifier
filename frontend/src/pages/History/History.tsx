...
export const History: React.FC = () => {
  ...
  const filtered = history.filter((item) => {
    if (levelFilter !== 'all' && item.confidence_level !== levelFilter)
      return false;
    if (
      searchQuery &&
      !item.bug_description.toLowerCase().includes(searchQuery.toLowerCase())
    )
      return false;
    return true;
  });
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            Complete audit log of all predictions, confidence levels, and human
            overrides
          </p>
        </div>
            ...
            {filtered.map((item) => (
              <tr key={item.id}>
                ...
                <td style={{ fontWeight: 600 }}>
                  {item.bug_description.slice(0, 70)}...
                </td>
