...
export const QuickPredict: React.FC = () => {
  ...
  const handleFeedback = async (isCorrect: boolean) => {
    ...
    if (isCorrect) {
      ...
      await logCorrection(
        ...
        'User flagged misclassification',
      );
  ...
  const handleExportJira = async () => {
    ...
    showToast(
      `Created Jira Ticket ${res.jira_issue_key} assigned to ${result.predicted_team}`,
    );
  };
  ...
  const copyResult = () => {
    ...
    navigator.clipboard.writeText(
      `${result.predicted_team} (Confidence: ${(result.confidence_score * 100).toFixed(1)}%)`,
    );
  ...
  const sampleBugs = [
    {
      title: 'User Login OAuth Error',
      desc: 'User login failed with HTTP 500 error when submitting OAuth token in authentication service',
    },
    {
      title: 'Database Pool Exhausted',
      desc: 'Database connection pool exhausted during high load queries on postgres cluster',
    },
    {
      title: 'React UI Overflow Issue',
      desc: 'React component CSS dropdown menu overflow issue in dark mode dashboard layout',
    },
    {
      title: 'Stripe Webhook Failure',
      desc: 'Stripe payment webhook verification failed due to missing signature header',
    },
    {
      title: 'K8s OOMKilled Pod Crash',
      desc: 'Kubernetes pod crashing with OOMKilled memory limit exception in deployment',
    },
  ];
    ...
    <div>
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            Real-time TF-IDF + Monte Carlo Dropout uncertainty estimation (&lt;
            500ms API SLA)
          </p>
        </div>
      </div>
      ...
      >
        ...
        <div className="card">
          ...
          <form onSubmit={handleSubmit}>
            ...
            >
              ...
              <span>
                {isPredicting
                  ? 'Calculating Monte Carlo Dropout...'
                  : 'Classify Bug Report'}
              </span>
            </button>
          </form>
        ...
        <div className="card">
          ...
          >
            <Brain size={18} color="var(--accent-purple)" /> Classification
            Result
          </h2>
          ...
          {!result ? (
            ...
            >
              ...
              <p style={{ fontWeight: 600, color: '#94a3b8' }}>
                Awaiting input submission
              </p>
              <p style={{ fontSize: '0.825rem', marginTop: '0.2rem' }}>
                Fill in the form to run variance-based MC Dropout prediction.
              </p>
            </div>
          ) : (
            <div>
              ...
              >
                  ...
                  >
                    <div
                      style={{
                        ...
                        background:
                          'linear-gradient(90deg, var(--accent-indigo), var(--accent-emerald))',
                      }}
                    />
                  </div>
                </div>
                ...
                >
                  <span>
                    MC Std Dev Uncertainty:{' '}
                    <strong>{result.uncertainty_score}</strong>
                  </span>
                  <span>
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                ...
                <button
                  className="btn-secondary"
                  style={{
                    color: feedbackLogged === 'correct' ? '#34d399' : undefined,
                  }}
                ...
                <button
                  className="btn-secondary"
                  style={{
                    color: feedbackLogged === 'wrong' ? '#fb7185' : undefined,
                  }}
